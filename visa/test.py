...
<script>
let controller = {}; // để cancel từng API riêng

// Hàm gọi API lấy danh sách cột
...
async function runProcess(type) {
  ...
  try {
    ...
    else out.textContent = data.log + "\\n📁 Tải: " + data.download_url;
  } catch (e) {
    out.textContent = "❌ Bị hủy hoặc lỗi mạng.";
  } finally {
    btn.disabled = false;
...
def run_nearby_like(df, lat_col, long_col, kw_col, key):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if res:
                df.at[idx, "Name"] = res[0].get("name","")
                df.at[idx, "Address"] = res[0].get("vicinity","")
        except Exception as e:
            logs.append(f"Dòng {idx+1}: {e}")
    return df, logs
...
def run_nearby():
    ...
    df, logs = run_nearby_like(df, lat_col, long_col, kw_col, GOOGLE_API_KEY)
